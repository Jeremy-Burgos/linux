...
Search from the root of the filesystem for a regular file
by name:
...
Use `find` when you care about correctness more than
convenience.

## 2. Finding files with `locate`

A faster indexed search:
...
This is often more convenient than `find`, but it depends
on an index that may not reflect the current state
immediately.

Use `locate` for speed and `find` for certainty.
...
Wildcards are useful. They are also dangerous when used
with deletion or move commands. Expand them mentally before
you trust them.

## 4. Process inspection with `ps`
...
This is a standard baseline view of what is currently
running.

## 5. Filtering process output with `grep`
...
* you still need to inspect PID, user, parent process,
  and path where relevant

## 6. Piping output

The pipe operator sends the output of one command to
another:
...
When investigating a service, process, or configuration
file, a simple pattern is:
...
You cannot minimize what you cannot find, and you cannot
trust what you have not inspected.